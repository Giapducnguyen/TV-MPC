#include "rtwtypes.h"
#include "pdist2_eiM5owPm.h"
#include <emmintrin.h>
#include "mwmathutil.h"
#include "rt_nonfinite.h"

void pdist2_eiM5owPm(const real_T Xin[200], const real_T Yin[200], const real_T
                     varargin_1[10], real_T D[400])
{
  __m128d tmp;
  real_T X[200];
  real_T Y[200];
  ...
  int32_T b_jj;
  int32_T b_qq;
  boolean_T logIndX[20];
  boolean_T logIndY[20];
  boolean_T exitg1;
  boolean_T nanflag;
  for (b_qq = 0; b_qq < 400; b_qq++) {
    D[b_qq] = (rtNaN);
  }

  for (b_qq = 0; b_qq < 20; b_qq++) {
    ...
    if (logIndY[b_qq]) {
      for (b_jj = 0; b_jj < 20; b_jj++) {
        if (logIndX[b_jj]) {
          ...
          D[b_jj + 20 * b_qq] = muDoubleScalarSqrt(tempSum);
        }
      }
    }
  }
