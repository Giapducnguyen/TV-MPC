#include "rtwtypes.h"
#include "computeGradLag_xG51nyih.h"
#include <string.h>

void computeGradLag_xG51nyih(real_T workspace[486], int32_T nVar, const real_T
  grad[486], const real_T AineqTrans[133164], const real_T AeqTrans[31104],
  ...
  int32_T mLB, const int32_T finiteUB[486], int32_T mUB, const real_T lambda[907])
{
  ...
  for (b = 0; b <= ix; b++) {
    iL0 = finiteLB[b];
    workspace[iL0 - 1] -= lambda[(mFixed + b) + 338];
  }

  if ((uint16_T)mLB - 1 < 0) {
    iL0 = mFixed + 338;
  } else {
    iL0 = (mFixed + (uint16_T)mLB) + 338;
  }
