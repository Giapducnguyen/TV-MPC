#include "rtwtypes.h"
#include "xzlarfg_w1qeJACP.h"
#include "xnrm2_fcWq9qrF.h"
#include "mwmathutil.h"
#include <emmintrin.h>

real_T xzlarfg_w1qeJACP(int32_T n, real_T *alpha1, real_T x[470596], int32_T ix0)
{
  __m128d tmp;
  real_T a;
  real_T tau;
  ...
  tau = 0.0;
  if (n > 0) {
    xnorm = xnrm2_fcWq9qrF(n - 1, x, ix0);
    if (xnorm != 0.0) {
      ...
      if (muDoubleScalarAbs(xnorm) < 1.0020841800044864E-292) {
        ...
        xnorm = muDoubleScalarHypot(*alpha1, xnrm2_fcWq9qrF(n - 1, x, ix0));
        if (*alpha1 >= 0.0) {
          xnorm = -xnorm;
        }
