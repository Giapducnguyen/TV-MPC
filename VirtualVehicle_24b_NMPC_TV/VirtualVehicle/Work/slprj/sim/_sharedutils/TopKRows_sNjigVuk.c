#include "rtwtypes.h"
#include "TopKRows_sNjigVuk.h"
#include "percolateDown_ErwJB4nk.h"
#include "sortLE_fTIm31wQ.h"

void TopKRows_sNjigVuk(const real_T X[502], real_T B[15], int32_T b_I[15])
{
  int32_T heap[15];
  int32_T b_ii;
  int32_T child;
  ...
  for (numToRemove = 6; numToRemove >= 0; numToRemove--) {
    percolateDown_ErwJB4nk(heap, numToRemove + 1, X);
  }

  for (numToRemove = 0; numToRemove < 487; numToRemove++) {
    if (numToRemove + 16 < heap[0]) {
      newRowIsHigher = sortLE_fTIm31wQ(X, numToRemove + 16, heap[0]);
    } else {
      newRowIsHigher = !sortLE_fTIm31wQ(X, heap[0], numToRemove + 16);
    }

    if (newRowIsHigher) {
      heap[0] = numToRemove + 16;
      percolateDown_ErwJB4nk(heap, 1, X);
    }
  }
