#include "rtwtypes.h"
#include "TopKRows_7X9flMG4.h"
#include "percolateDown_tPShsN4f.h"
#include "sortLE_L35ZBP8I.h"

void TopKRows_7X9flMG4(const real_T X[354], real_T B[15], int32_T b_I[15])
{
  int32_T heap[15];
  int32_T b_ii;
  int32_T child;
  ...
  for (numToRemove = 6; numToRemove >= 0; numToRemove--) {
    percolateDown_tPShsN4f(heap, numToRemove + 1, X);
  }

  for (numToRemove = 0; numToRemove < 339; numToRemove++) {
    if (numToRemove + 16 < heap[0]) {
      newRowIsHigher = sortLE_L35ZBP8I(X, numToRemove + 16, heap[0]);
    } else {
      newRowIsHigher = !sortLE_L35ZBP8I(X, heap[0], numToRemove + 16);
    }

    if (newRowIsHigher) {
      heap[0] = numToRemove + 16;
      percolateDown_tPShsN4f(heap, 1, X);
    }
  }
