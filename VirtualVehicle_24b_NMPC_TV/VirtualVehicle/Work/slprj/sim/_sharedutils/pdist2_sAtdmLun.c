#include "rtwtypes.h"
#include "pdist2_sAtdmLun.h"
#include <emmintrin.h>
#include "mwmathutil.h"
#include "rt_nonfinite.h"

void pdist2_sAtdmLun(const real_T Xin[120], const real_T Yin[120], const real_T
                     varargin_1[10], real_T D[144])
{
  __m128d tmp;
  real_T X[120];
  real_T Y[120];
  ...
  int32_T b_jj;
  int32_T b_qq;
  boolean_T logIndX[12];
  boolean_T logIndY[12];
  boolean_T exitg1;
  boolean_T nanflag;
  for (b_qq = 0; b_qq < 144; b_qq++) {
    D[b_qq] = (rtNaN);
  }

  for (b_qq = 0; b_qq < 12; b_qq++) {
    ...
    if (logIndY[b_qq]) {
      for (b_jj = 0; b_jj < 12; b_jj++) {
        if (logIndX[b_jj]) {
          ...
          D[b_jj + 12 * b_qq] = muDoubleScalarSqrt(tempSum);
        }
      }
    }
  }
