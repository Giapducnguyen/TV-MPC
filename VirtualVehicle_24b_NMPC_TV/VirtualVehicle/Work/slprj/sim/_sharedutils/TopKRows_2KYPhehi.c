#include "rtwtypes.h"
#include "TopKRows_2KYPhehi.h"
#include "percolateDown_nblHrqX3.h"
#include "sortLE_r9ISyzQw.h"

void TopKRows_2KYPhehi(const real_T X[503], real_T B[6], int32_T b_I[6])
{
  int32_T heap[6];
  int32_T b_ii;
  int32_T child;
  ...
  percolateDown_nblHrqX3(heap, 3, X);
  percolateDown_nblHrqX3(heap, 2, X);
  percolateDown_nblHrqX3(heap, 1, X);
  for (numToRemove = 0; numToRemove < 497; numToRemove++) {
    if (numToRemove + 7 < heap[0]) {
      newRowIsHigher = sortLE_r9ISyzQw(X, numToRemove + 7, heap[0]);
    } else {
      newRowIsHigher = !sortLE_r9ISyzQw(X, heap[0], numToRemove + 7);
    }
