#include "rtwtypes.h"
#include "TopKRows_T1MA2wQO.h"
#include "percolateDown_yjsIsF2e.h"
#include "sortLE_yzp2vRpo.h"

void TopKRows_T1MA2wQO(const real_T X[472], real_T B[10], int32_T b_I[10])
{
  int32_T heap[10];
  int32_T b_ii;
  int32_T child;
  ...
  for (numToRemove = 4; numToRemove >= 0; numToRemove--) {
    percolateDown_yjsIsF2e(heap, numToRemove + 1, X);
  }

  for (numToRemove = 0; numToRemove < 462; numToRemove++) {
    if (numToRemove + 11 < heap[0]) {
      newRowIsHigher = sortLE_yzp2vRpo(X, numToRemove + 11, heap[0]);
    } else {
      newRowIsHigher = !sortLE_yzp2vRpo(X, heap[0], numToRemove + 11);
    }

    if (newRowIsHigher) {
      heap[0] = numToRemove + 11;
      percolateDown_yjsIsF2e(heap, 1, X);
    }
  }
