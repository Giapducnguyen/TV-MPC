...
void xgeqp3_tpQGeAH7(real_T A[68121], int32_T m, int32_T n, int32_T jpvt[261],
                     real_T tau[261])
{
  int32_T i;
  int32_T minmn;
  ...
  if (minmn < 1) {
    ...
    for (i = 0; i < n; i++) {
      if (jpvt[i] != 0) {
        nfxd++;
        if (i + 1 != nfxd + 1) {
          xswap_yckutoLa(m, A, i * 261 + 1, nfxd * 261 + 1);
          jpvt[i] = jpvt[nfxd];
          jpvt[nfxd] = i + 1;
        } else {
          jpvt[i] = i + 1;
    ...
    qrf_keBWVM91(A, m, n, nfxd, tau);
    if (nfxd < minmn) {
      qrpf_WMG9F5uv(A, m, n, nfxd, tau, jpvt);
    }
  }
}
