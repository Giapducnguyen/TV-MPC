...
void xgeqp3_YKRtYU1R(real_T A[105625], int32_T m, int32_T n, int32_T jpvt[325],
                     real_T tau[325])
{
  int32_T i;
  int32_T minmn;
  ...
  if (minmn < 1) {
    ...
    for (i = 0; i < n; i++) {
      if (jpvt[i] != 0) {
        nfxd++;
        if (i + 1 != nfxd + 1) {
          xswap_kmRB7nyJ(m, A, i * 325 + 1, nfxd * 325 + 1);
          jpvt[i] = jpvt[nfxd];
          jpvt[nfxd] = i + 1;
        } else {
          jpvt[i] = i + 1;
    ...
    qrf_2JHfYBns(A, m, n, nfxd, tau);
    if (nfxd < minmn) {
      qrpf_AQSdGH4Z(A, m, n, nfxd, tau, jpvt);
    }
  }
}
