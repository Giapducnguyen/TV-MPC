#include "rtwtypes.h"
#include "TopKRows_sRtALvi6.h"
#include "percolateDown_0nfY5RnN.h"
#include "sortLE_r9ISyzQw.h"

void TopKRows_sRtALvi6(const real_T X[503], real_T B[10], int32_T b_I[10])
{
  int32_T heap[10];
  int32_T b_ii;
  int32_T child;
  ...
  for (numToRemove = 4; numToRemove >= 0; numToRemove--) {
    percolateDown_0nfY5RnN(heap, numToRemove + 1, X);
  }

  for (numToRemove = 0; numToRemove < 493; numToRemove++) {
    if (numToRemove + 11 < heap[0]) {
      newRowIsHigher = sortLE_r9ISyzQw(X, numToRemove + 11, heap[0]);
    } else {
      newRowIsHigher = !sortLE_r9ISyzQw(X, heap[0], numToRemove + 11);
    }

    if (newRowIsHigher) {
      heap[0] = numToRemove + 11;
      percolateDown_0nfY5RnN(heap, 1, X);
    }
  }
