#include "rtwtypes.h"
#include "pdist2_JBu1Z7b9.h"
#include <emmintrin.h>
#include "mwmathutil.h"
#include "rt_nonfinite.h"

void pdist2_JBu1Z7b9(const real_T Xin[60], const real_T Yin[60], const real_T
                     varargin_1[10], real_T D[36])
{
  __m128d tmp;
  real_T X[60];
  real_T Y[60];
  ...
  boolean_T exitg1;
  boolean_T nanflag;
  for (b_qq = 0; b_qq < 36; b_qq++) {
    D[b_qq] = (rtNaN);
  }

  for (b_qq = 0; b_qq < 6; b_qq++) {
    ...
    if (logIndY[b_qq]) {
      for (b_jj = 0; b_jj < 6; b_jj++) {
        if (logIndX[b_jj]) {
          ...
          D[b_jj + 6 * b_qq] = muDoubleScalarSqrt(tempSum);
        }
      }
    }
  }
