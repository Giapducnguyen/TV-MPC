#include "rtwtypes.h"
#include "pdist2_wL0bKCme.h"
#include <emmintrin.h>
#include "mwmathutil.h"
#include "rt_nonfinite.h"

void pdist2_wL0bKCme(const real_T Xin[126], const real_T Yin[126], const real_T
                     varargin_1[7], real_T D[324])
{
  __m128d tmp;
  real_T X[126];
  real_T Y[126];
  ...
  int32_T b_jj;
  int32_T b_qq;
  boolean_T logIndX[18];
  boolean_T logIndY[18];
  boolean_T exitg1;
  boolean_T nanflag;
  for (b_qq = 0; b_qq < 324; b_qq++) {
    D[b_qq] = (rtNaN);
  }

  for (b_qq = 0; b_qq < 18; b_qq++) {
    ...
    if (logIndY[b_qq]) {
      for (b_jj = 0; b_jj < 18; b_jj++) {
        if (logIndX[b_jj]) {
          ...
          D[b_jj + 18 * b_qq] = muDoubleScalarSqrt(tempSum);
        }
      }
    }
  }
