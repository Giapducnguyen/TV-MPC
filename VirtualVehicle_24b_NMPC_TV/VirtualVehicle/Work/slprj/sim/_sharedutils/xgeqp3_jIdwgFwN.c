...
void xgeqp3_jIdwgFwN(real_T A[231361], int32_T m, int32_T n, int32_T jpvt[481],
                     real_T tau[481])
{
  int32_T i;
  int32_T minmn;
  ...
  if (minmn < 1) {
    ...
    for (i = 0; i < n; i++) {
      if (jpvt[i] != 0) {
        nfxd++;
        if (i + 1 != nfxd + 1) {
          xswap_YS1Or0II(m, A, i * 481 + 1, nfxd * 481 + 1);
          jpvt[i] = jpvt[nfxd];
          jpvt[nfxd] = i + 1;
        } else {
          jpvt[i] = i + 1;
    ...
    qrf_8TSzG9wp(A, m, n, nfxd, tau);
    if (nfxd < minmn) {
      qrpf_QdrrEHxX(A, m, n, nfxd, tau, jpvt);
    }
  }
}
