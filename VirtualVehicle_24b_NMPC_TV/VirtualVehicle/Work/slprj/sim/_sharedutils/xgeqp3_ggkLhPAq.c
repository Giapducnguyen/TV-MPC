...
void xgeqp3_ggkLhPAq(real_T A[145161], int32_T m, int32_T n, int32_T jpvt[381],
                     real_T tau[381])
{
  int32_T i;
  int32_T minmn;
  ...
  if (minmn < 1) {
    ...
    for (i = 0; i < n; i++) {
      if (jpvt[i] != 0) {
        nfxd++;
        if (i + 1 != nfxd + 1) {
          xswap_vFpGBkoU(m, A, i * 381 + 1, nfxd * 381 + 1);
          jpvt[i] = jpvt[nfxd];
          jpvt[nfxd] = i + 1;
        } else {
          jpvt[i] = i + 1;
    ...
    qrf_dVjkUEPW(A, m, n, nfxd, tau);
    if (nfxd < minmn) {
      qrpf_Xa90fRPJ(A, m, n, nfxd, tau, jpvt);
    }
  }
}
