#include "rtwtypes.h"
#include "TopKRows_viuFn9ZS.h"
#include "percolateDown_gKA0lw1N.h"
#include "sortLE_fTIm31wQ.h"

void TopKRows_viuFn9ZS(const real_T X[502], real_T B[10], int32_T b_I[10])
{
  int32_T heap[10];
  int32_T b_ii;
  int32_T child;
  ...
  for (numToRemove = 4; numToRemove >= 0; numToRemove--) {
    percolateDown_gKA0lw1N(heap, numToRemove + 1, X);
  }

  for (numToRemove = 0; numToRemove < 492; numToRemove++) {
    if (numToRemove + 11 < heap[0]) {
      newRowIsHigher = sortLE_fTIm31wQ(X, numToRemove + 11, heap[0]);
    } else {
      newRowIsHigher = !sortLE_fTIm31wQ(X, heap[0], numToRemove + 11);
    }

    if (newRowIsHigher) {
      heap[0] = numToRemove + 11;
      percolateDown_gKA0lw1N(heap, 1, X);
    }
  }
