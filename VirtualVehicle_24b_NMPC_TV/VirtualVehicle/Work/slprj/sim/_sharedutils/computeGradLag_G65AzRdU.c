#include "rtwtypes.h"
#include "computeGradLag_G65AzRdU.h"
#include <string.h>

void computeGradLag_G65AzRdU(real_T workspace[423], int32_T nVar, const real_T
  grad[423], const real_T AineqTrans[118440], const real_T AeqTrans[13113],
  ...
  int32_T mLB, const int32_T finiteUB[423], int32_T mUB, const real_T lambda[814])
{
  ...
  for (b = 0; b <= ix; b++) {
    iL0 = finiteLB[b];
    workspace[iL0 - 1] -= lambda[(mFixed + b) + 311];
  }

  if ((uint16_T)mLB - 1 < 0) {
    iL0 = mFixed + 311;
  } else {
    iL0 = (mFixed + (uint16_T)mLB) + 311;
  }
