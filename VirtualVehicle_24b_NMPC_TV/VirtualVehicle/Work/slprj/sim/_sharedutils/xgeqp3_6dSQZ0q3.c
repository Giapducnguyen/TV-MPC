...
void xgeqp3_6dSQZ0q3(real_T A[68644], int32_T m, int32_T n, int32_T jpvt[262],
                     real_T tau[262])
{
  int32_T i;
  int32_T minmn;
  ...
  if (minmn < 1) {
    ...
    for (i = 0; i < n; i++) {
      if (jpvt[i] != 0) {
        nfxd++;
        if (i + 1 != nfxd + 1) {
          xswap_KPPuRq1U(m, A, i * 262 + 1, nfxd * 262 + 1);
          jpvt[i] = jpvt[nfxd];
          jpvt[nfxd] = i + 1;
        } else {
          jpvt[i] = i + 1;
    ...
    qrf_Xxtq0qdL(A, m, n, nfxd, tau);
    if (nfxd < minmn) {
      qrpf_THVuiHvA(A, m, n, nfxd, tau, jpvt);
    }
  }
}
