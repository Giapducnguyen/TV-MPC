...
void xgeqp3_RiHZ4vn8(real_T A[51529], int32_T m, int32_T n, int32_T jpvt[227],
                     real_T tau[227])
{
  int32_T i;
  int32_T minmn;
  ...
  if (minmn < 1) {
    ...
    for (i = 0; i < n; i++) {
      if (jpvt[i] != 0) {
        nfxd++;
        if (i + 1 != nfxd + 1) {
          xswap_oMzfE4n9(m, A, i * 227 + 1, nfxd * 227 + 1);
          jpvt[i] = jpvt[nfxd];
          jpvt[nfxd] = i + 1;
        } else {
          jpvt[i] = i + 1;
    ...
    qrf_ys5Slopk(A, m, n, nfxd, tau);
    if (nfxd < minmn) {
      qrpf_MirdViFH(A, m, n, nfxd, tau, jpvt);
    }
  }
}
