#include "rtwtypes.h"
#include "computeGradLag_7uAKL99j.h"
#include <string.h>

void computeGradLag_7uAKL99j(real_T workspace[486], int32_T nVar, const real_T
  grad[486], const real_T AineqTrans[131220], const real_T AeqTrans[32076],
  ...
  int32_T mLB, const int32_T finiteUB[486], int32_T mUB, const real_T lambda[905])
{
  ...
  for (b = 0; b <= ix; b++) {
    iL0 = finiteLB[b];
    workspace[iL0 - 1] -= lambda[(mFixed + b) + 336];
  }

  if ((uint16_T)mLB - 1 < 0) {
    iL0 = mFixed + 336;
  } else {
    iL0 = (mFixed + (uint16_T)mLB) + 336;
  }
